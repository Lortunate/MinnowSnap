...
use crate::core::{i18n, ocr_service, ocr_service::OcrModelStatus};
...
pub(crate) fn snapshot(state: &PreferencesState) -> OcrSnapshot {
    OcrSnapshot {
        enabled: ocr_service::is_enabled(),
        status: ocr_service::current_status(
...
mod tests {
    use super::*;
    use crate::ui::preferences::session::state::PreferencesNotice;
