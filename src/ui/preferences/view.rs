use super::{
    render::{self, PreferencesRenderActions},
    session::{
        self, MutationResult,
...
impl PreferencesView {
    ...
    pub(super) fn on_language_selected(value: SharedString, _: &mut Window, cx: &mut App) {
        Self::apply_app_mutation(session::general::set_language(value), cx);
    }

    pub(super) fn on_theme_selected(value: SharedString, window: &mut Window, cx: &mut App) {
        Self::apply_app_mutation(session::general::set_theme(value, window, cx), cx);
    }

    pub(super) fn on_font_selected(value: SharedString, _: &mut Window, cx: &mut App) {
        Self::apply_app_mutation(session::general::set_font(value, cx), cx);
    }
    ...
    pub(super) fn on_shortcut_key_down(&mut self, event: &KeyDownEvent, _: &mut Window, cx: &mut Context<Self>) {
        ...
        self.state.stop_shortcut_recording();
        let current = session::shortcuts::snapshot(cx).bindings;
        let updated = session::shortcuts::next_shortcut_bindings(&current, target_action, &formatted);
    ...
    fn apply_shortcuts(&mut self, bindings: ShortcutBindings, cx: &mut Context<Self>) {
        match session::shortcuts::persist_shortcut_bindings(bindings, cx) {
    ...
    fn download_ocr_models(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        cx.spawn_in(window, move |this: WeakEntity<PreferencesView>, cx: &mut AsyncWindowContext| {
            ...
            async move {
                ...
                let download = ocr_service::download_mobile_models(true, Some(progress_callback));
                tokio::pin!(download);
    ...
    fn browse_save_path(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        cx.spawn_in(window, move |this: WeakEntity<PreferencesView>, cx: &mut AsyncWindowContext| {
            ...
            async move {
                ...
                let _ = this.update(&mut cx, |view: &mut PreferencesView, cx| {
                    view.apply_mutation(session::general::set_save_path(save_path), cx);
                });
            }
        })
        .detach();
    ...
    fn render_sidebar_items(&self, frame: &PreferencesFrame, cx: &mut Context<Self>) -> Vec<AnyElement> {
        frame
            ...
            .map(|item| {
                let page = item.page;
                render::components::sidebar_item(item, cx)
        ...
        let mut panel = div()
            ...
            .child(
                div()
                    ...
                    .bg(theme.popover)
                    .overflow_hidden()
                    .child(render::chrome::sidebar_panel(sidebar_items, cx))
                    .child(render::chrome::content_panel(
                        frame.page_title.clone(),
                        render::components::close_button(|_, window, cx| Self::request_close(window, cx), cx),
                        page_notice,
                        page_body,
                        cx,
                    )),
    ...
    fn build_frame(&self, cx: &App) -> PreferencesFrame {
        session::frame::build(&self.state, cx)
    }
}
