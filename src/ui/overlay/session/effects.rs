...
use crate::core::capture::action::{ActionResult, CaptureAction};
use crate::core::i18n;
use crate::core::io::clipboard::copy_text_to_clipboard;
use crate::core::notify::NotificationType;
use crate::ui::long_capture::{self, LongCaptureRequest};
...
impl OverlayHandle {
    pub(crate) fn dispatch(&self, command: crate::ui::overlay::session::OverlayCommand, window: &mut Window, cx: &mut App) {
    ...
    pub(crate) fn prepare_frame(&self, window: &Window, cx: &mut App) -> crate::ui::overlay::session::OverlayFrame {
        self.sync_viewport(window, cx);
    ...
    fn start_long_capture(
        ...
        viewport_rect: crate::core::geometry::RectF,
        viewport_scale: f64,
        window: &mut Window,
        cx: &mut App,
    ) {
    ...
    fn copy_text(&self, payload: CopyTextPayload, window: &mut Window, cx: &mut App) {
        if copy_text_to_clipboard(payload.text) {
            crate::core::notify::show(&payload.title, &payload.message, payload.notification_type);
    ...
    fn capture(&self, action: CaptureAction, context: crate::core::capture::action::ActionContext, window: &mut Window, cx: &mut App) {
